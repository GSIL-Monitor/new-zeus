...
public class Master {
	...
	private final ConcurrentHashMap<String,String> runningJobs = new ConcurrentHashMap<String, String>() ;

	private final int maxRunningJobs = Environment.getMaxRunningJob();
	...
	public Master(final MasterContext context) {
		...
		}, 1, Environment.getActionInterval(), TimeUnit.MINUTES);
...
//						log.info("roll back lost jobID :" + id.toString());
	...
	private synchronized MasterWorkerHolder getRunableWorker(String hostGroupId) {
		...
		if (context.getHostGroupCache()!=null) {
			...
			log.info("hostGroupId: " + hostGroupId + " hostGroupCache:" + JsonUtil.bean2json(hostGroupCache));
			if (hostGroupCache != null && hostGroupCache.getHosts()!=null && hostGroupCache.getHosts().size()>0) {
				...
				for (int i = 0; i < size && selectWorker == null; i++) {
					...
					for (MasterWorkerHolder worker : context.getWorkers().values()) {
						try {
						    if(worker != null){
                                log.info("worker :" + JsonUtils.objectToJson(worker.getHeart()));
                            }else{
						        log.error("没有可用的 worker 注册到 master 上");
                            }
			    ...
			    log.error("host group :" + hostGroupId + " 不包含 host ");
            }
		}else{
		    log.error("host group 数据为空");
        }
	...
	private boolean checkMaxRunning(){
        int  runningJobCount = runningJobs.size() ;
        log.info("运行:" + runningJobCount);
        if(runningJobCount >= maxRunningJobs){
            log.info("运行中的 job 数量:" + runningJobCount + " 大于最大任务数:" + maxRunningJobs);
            return false ;
        }
        return true ;
    }
	...
	private void scan() {
		if (!context.getQueue().isEmpty() && checkMaxRunning()) {
	...
	private void scanExceptionQueue(){
		if(!context.getExceptionQueue().isEmpty() && checkMaxRunning()){
	...
	private void runScheduleJob(MasterWorkerHolder selectWorker,final String jobID) {
		...
		new Thread() {
			...
			public void run() {
				...
				try{
				    runningJobs.putIfAbsent(jobID, w.getHeart().host) ;
	...
	private void runScheduleJobContext(MasterWorkerHolder w, final String jobID, int runCount, final int rollBackTimes, final int rollBackWaitTime){
        ...
        log.info("job:" + jobID + " 第" + runCount + " 次运行");
...
/*							actionPer.setScript(jobDetail.getScript());*/
							...
							try {
								log.info("定时任务JobId: " + jobDetail.getId()+";  ActionId: " +actionPer.getId() + " 保存到 action 表");
									context.getGroupManager().saveJob(actionPer);
									actionDetails.put(actionPer.getId(),actionPer);
								//}
							} catch (ZeusException e) {
								log.error("定时任务JobId:" + jobDetail.getId() + " 生成Action 到 action 表 " +actionPer.getId() + "失败", e);
							}
						}
					}
				}catch(Exception ex){
...
/*									actionPer.setScript(jobDetail.getScript());*/
									...
									try {
										if(!actionDetails.containsKey(actionPer.getId())){
											log.info("依赖任务JobId: " + jobDetail.getId()+";  ActionId: " +actionPer.getId() + " 保存到 action 表");
												context.getGroupManager().saveJob(actionPer);
											actionDetails.put(actionPer.getId(),actionPer);
										}
									} catch (ZeusException e) {
